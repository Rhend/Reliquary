...
#   • voies scellées : ouverture manuelle refusée sans objet, acceptée avec,
#     signal + persistance round-trip, compteur source unique,
#   • Atelier (Forge) accessible sans condition (gate Village mort),
...
# Remise à zéro de l'état chantier 11/12 entre les tests.
func _reset_etat() -> void:
	...
	GameData.player["voies_ouvertes"] = {}
...
# ─── 8. Voies : refus sans objet, ouverture manuelle, persistance ──

func _test_voies() -> void:
	print("\n[TEST 8] Voies : refus sans objet, clic avec objet, round-trip")
	_reset_etat()
	_assert(not GameData.ouvrir_voie("biome_foret"), "sans objet → ouverture refusée")
	_assert(GameData.nb_voies_ouvertes() == 0, "compteur à 0")
	GameData.player["objets_lieutenants"] = {"biome_foret": true}
	var signaux: Array = []
	var cb := func(lieu: String) -> void: signaux.append(lieu)
	EventBus.voie_ouverte.connect(cb)
	_assert(GameData.ouvrir_voie("biome_foret"), "avec objet → voie ouverte")
	_assert(GameData.voie_ouverte("biome_foret"), "état persistant en mémoire")
	_assert(signaux == ["biome_foret"], "signal voie_ouverte émis une fois")
	_assert(not GameData.ouvrir_voie("biome_foret"), "déjà ouverte → refus (pas de double)")
	_assert(signaux.size() == 1, "pas de second signal")
	_assert(GameData.nb_voies_ouvertes() == 1, "compteur source unique = 1")
	...
	GameData.player["objets_lieutenants"] = {}
	SaveManager.recharger()
	_assert(GameData.voie_ouverte("biome_foret"), "voie survivante au round-trip disque")
	_assert(GameData.possede_objet_lieutenant("biome_foret"), "objet survivant au round-trip")
	_reset_etat()

# ─── 9. Atelier accessible sans condition ───────────────────

func _test_atelier_ouvert() -> void:
	print("\n[TEST 9] Atelier (Forge) ouvert d'emblée — plus aucun gate")
	var forge_min := -1
	var voies_min := -1
	for d: Array in Village.MENU_ITEMS:
		...
		if d[4] == "voies":
			voies_min = int(d[2])
	_assert(forge_min == 0, "hex FORGE présent dès le Village T0 (tier_min 0)")
	_assert(voies_min == 0, "hex VOIES présent dès le Village T0 (tier_min 0)")
	# Rendu du panneau Forge à Village T0 (l'ancien verrou affichait un mur).
	GameData.village["maitrise_actuelle"] = 0
	var v := Village.new()
	v.rp_content = VBoxContainer.new()
	ForgePanel.build(v)
	_assert(v.rp_content.get_child_count() > 0,
			"panneau Forge rendu à Village T0 (contenu, pas de mur de verrou)")
	v.rp_content.free()
	v.free()

# ─── 10. Panneau VOIES : rendu + clic « Restaurer » ─────────

func _test_panneau_voies() -> void:
	print("\n[TEST 10] Panneau VOIES : 6 voies, bouton Restaurer avec objet")
	_reset_etat()
	var v := Village.new()
	v.rp_content = VBoxContainer.new()
	VoiesPanel.build(v)
	_assert(v.rp_content.get_child_count() > 0, "panneau VOIES rendu")
	_assert(VoiesPanel.DESTINATIONS.lieutenants_par_lieu.size() == 6,
			"6 voies (une par Lieutenant mappé)")
	_assert(_bouton_contenant(v.rp_content, Translations.T("voies.restaurer_btn")) == null,
			"sans objet : aucun bouton Restaurer")
	v.rp_content.free()
	...
	_assert(btn != null, "avec objet : bouton Restaurer présent (prêt → clic)")
	if btn != null:
		btn.pressed.emit()
		_assert(GameData.voie_ouverte("biome_foret"), "clic → voie ouverte")
	v.rp_content.free()
	v.free()
	_reset_etat()
