...
class CircleRing extends Control:
	...
	var ring_radius   := 0.0    # 0 = calculé depuis size, >0 = valeur explicite
	var ring_width    := 13.0
	...
	func _draw() -> void:
		var c := size * 0.5
		var r := ring_radius if ring_radius > 0.0 else minf(size.x, size.y) * 0.5 - ring_width * 0.5
		...
		# Outline extérieur (bord foncé)
		draw_arc(c, r + ring_width * 0.5 + 0.5, 0.0, TAU, 64, Color(0, 0, 0, 0.55), 1.5, true)
		# Outline intérieur (bord foncé)
		draw_arc(c, r - ring_width * 0.5 - 0.5, 0.0, TAU, 64, Color(0, 0, 0, 0.55), 1.5, true)
		# Liseré lumineux sur le bord extérieur
		var rim := ring_color.lightened(0.3); rim.a = 0.50
		draw_arc(c, r + ring_width * 0.5 - 1.0, 0.0, TAU, 64, rim, 1.0, true)
...
func _build_hub(creature: Dictionary, tier: int) -> void:
	...
	var diam   := RING_RADIUS * 2.0 + 70.0   # marge pour corona + sparkles
