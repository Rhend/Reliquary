...
| Sprite Spine ANIMÉ du personnage principal (« Relic », export WIP de Christophe — 4 animations : `Idle` en boucle, `Attack_CaC` sur son action, `Hit` quand il est touché, `Death` à sa mort — pose finale TENUE, pas de fondu) dans la scène de bataille CTB. L'attaque du JOUEUR déclenche un zoom-DUEL façon Darkest Dungeon (les deux personnages convergent au centre, constantes `CombatCtbUi.DUEL_*` — activations ennemies sans effet de caméra). Vérif runtime + assets : `tools/verif_spine.gd`. Runtime **spine-godot GDExtension** dans `bin/` (versionné : windows + linux seulement, branche Spine 4.3, buildé pour Godot 4.6.3 mais forward-compatible — vérifié OK sous 4.7.2 — source officielle EsotericSoftware ; si l'extension refuse de charger, chercher d'abord un résidu `bin/windows/~*.TMP` laissé par une copie verrouillée, pas un problème d'ABI). Assets : `assets/personnages/relic/` (la 1re ligne du `.atlas` doit nommer le `.png` réel du dossier). Dossier de dépôt artiste `ChristopheAnimationWIP/` = zone d'échange NON importée (`.gdignore`) — intégrer = copier dans `assets/` + corriger l'atlas. Le script ne référence JAMAIS les classes Spine par leur type (ClassDB uniquement) : sans extension, `creer()` rend null et l'UI garde son placeholder EnergyBoule | `scenes/combat_ctb/SpriteSpinePersonnage.gd` |
...
| **ShowRoom** (vitrine dev des assets Spine, 08/2026) : `scenes/showroom/ShowRoom.tscn`, atteinte par le bouton dev « ShowRoom » du hub (`Village.DEBUG_SHOWROOM_BTN`, Échap revient au QG via la static `ShowRoom.scene_retour`) ou lancée seule (Échap quitte alors) ; n'écrit JAMAIS la sauvegarde. Mode LIBRE = fond neutre + repère de sol, une RANGÉE par personnage — le HÉROS d'abord (mètre étalon), puis les monstres, chacun montrant ses apparences de gauche à droite (5 paliers Commun→Légendaire pour un ennemi) —, caméra pan/zoom ; mode COMBAT = cadrage RÉEL (`SOL_Y_FRAC`/`SOL_X_*`/`HAUTEUR_CIBLE_PX` recopiés de CombatCtbUi) du monstre courant face au héros sur le décor city, `←/→` monstre, `↑/↓` palier, `H` apparence du héros. ÉCLAIRAGE `B` (4 niveaux Nuit/Studio/Jour/Blanc, défaut Studio — le fond quasi noir d'origine noyait les paliers Commun) : il agit sur le FOND en libre et sur un VOILE posé au-dessus du décor en combat — **jamais sur les personnages** (les moduler fausserait ce qu'on vient juger). Les textes s'assombrissent au-delà de `SEUIL_FOND_CLAIR`. Contenu lu du registre `data/personnages/spine_personnages.tres` (`SpinePersonnagesData`) : **un monstre livré = une entrée, rien d'autre à toucher**. Captures : `SHOT_MODE=showroom` (ScreenshotTool). ⚠ `DECOR_SOL_FRAC = 0.688` — le sol du décor city NE tombe PAS sur `SOL_Y_FRAC = 0.60` : la ShowRoom recale et redimensionne le décor pour que les pieds posent ; le même arbitrage se posera à l'intégration en combat réel (bouger le décor ou bouger SOL_Y_FRAC) | `scenes/showroom/ShowRoom.gd` |
| Assets Spine des ENNEMIS (08/2026) : `assets/personnages/ennemis/<id>/` — FlameBot + WorkBot livrés. MÊME contrat d'animation que Relic (`Idle`/`Attack_CaC`/`Hit`/`Death`), et les 5 PALIERS sont des **skins Spine** `<Nom>_Nv1`…`_Nv5` (Nv1 = Commun … Nv5 = Légendaire) commutées par `SpriteSpinePersonnage.definir_skin()` (`set_skin_by_name` + `set_slots_to_setup_pose` — sans le 2ᵉ appel la pose garde l'ancienne apparence). `creer()` prend désormais skel/atlas/skin/hauteur en paramètres, Relic restant le défaut. Inspecter une livraison : `tools/inspect_spine_ennemis.gd`. Les apparences d'une entrée passent par `SpinePersonnagesData.apparences()` : `variantes` (liste nommée) > `prefixe_skin` (5 paliers) > apparence unique — un appelant n'a jamais à savoir laquelle. ⚠ Relic n'expose QU'UNE skin « default » (vérifié 24/08/2026) : les versions masculine/féminine du héros N'ONT PAS été livrées ; le jour où elles arrivent, remplir `variantes` dans le `.tres` suffit (la touche `H` de la ShowRoom les cyclera). ⚠ Les ennemis ne sont PAS encore branchés au combat (toujours des placeholders EnergyBoule) | `scripts/resources/SpinePersonnagesData.gd` |
...
godot --headless --path . res://tests/TestShowRoom.tscn         # vitrine Spine : registre, aller-retour QG, éclairage (38)
