...
- **La ShowRoom EST le combat réel, fusionnée (09/2026)** — supersède l'ancien
  principe « banc d'essai, pas une fin » (acté 2026-08-27) : celui-ci demandait de
  répercuter à la main tout réglage de la vitrine vers `CombatCtbUi`, ce qui a
  fini par doubler le travail (retour Rhend : « c'est censé être la même UI »).
  `ShowRoom.gd` n'a PLUS sa propre reconstruction de décor/sprites/file : elle
  INSTANCIE le VRAI `CombatCtbUi` (`ShowRoom._lancer_duel`, faux combat héros
  RÉEL vs la créature choisie, aucune récompense câblée). Attaquer/Défendre/
  Compétence/cible sont les VRAIS boutons — plus aucune re-simulation du
  zoom-duel ou de la file d'initiative côté vitrine, `TestCombatUi.gd` couvre déjà
  ce rendu-là. Ne reste PROPRE À LA VITRINE (surcouche dev, jamais dans le vrai
  combat) que la navigation : ←/→ et ↑/↓ (créature/palier prévisualisés — surcharges
  `CombatCtbUi.previsu_palier_ennemi`/`previsu_niveau_heros`/`previsu_cosmetique_heros`,
  -1/0 = comportement réel, ignorées par le jeu), `H`/`V` (niveau/visage du héros),
  `B` (éclairage du décor via `CombatCtbUi.previsu_definir_voile`, jamais les
  personnages), `Tab` (décor Usine seul ⇄ duel), et `F1` — un mémo des commandes
  ci-dessus, MASQUÉ par défaut pour que la vitrine n'ait l'air différent du vrai
  combat que si on le demande. Le mode LIBRE (toutes les créatures en rangées) a
  été SUPPRIMÉ : aucun équivalent possible dans un écran de combat réel, jugé par
  Rhend comme n'étant pas un doublon à préserver — pour comparer plusieurs
  paliers/costumes il faut maintenant naviguer au clavier dans le duel. ⚠ Dette
  restante à cette date : le décor adverse (Usine, `CombatDecorFactory`,
  28/08/2026) n'est TOUJOURS PAS dynamique selon le Lieu — un seul décor adverse
  livré à ce jour, affiché pour TOUS les combats (a remplacé le placeholder
  `BiomeBackground("forest")`, lui-même déjà affiché sans distinction de Lieu —
  pas une régression, juste pas encore résolu). Le sprite des adversaires, lui,
  est BRANCHÉ (09/2026, avant la fusion) : `CombatCtbUi._creer_sprite_ennemi`
  pioche dans `SpinePersonnagesData` par id, générique à toute future créature —
  `EnergyBoule` ne reste qu'un repli pour une créature sans livraison Spine. Le
  miroir des ennemis est RÉGLÉ (voir `regarde_a_droite` ci-dessous).
...
| **ShowRoom** (vitrine dev des assets Spine, FUSIONNÉE avec `CombatCtbUi` le 09/2026 — voir « La ShowRoom EST le combat réel » ci-dessus) : `scenes/showroom/ShowRoom.tscn` (racine `Control`, plus `Node2D` depuis la fusion), atteinte par le bouton dev « ShowRoom » du hub (`Village.DEBUG_SHOWROOM_BTN`, Échap revient au QG via la static `ShowRoom.scene_retour`) ou lancée seule (Échap quitte alors) ; n'écrit JAMAIS la sauvegarde (`SaveManager.load_save()` appelé en LECTURE seule pour le héros réel quand lancée seule — même garde que `SandboxExpe._avatar_choisi`). Mode DUEL (défaut) = `ShowRoom._lancer_duel` monte un `CtbMoteur` factice (héros RÉEL via `CtbPont.combattant_depuis_heros` vs la créature du registre courante via `CtbPont.combattant_depuis_entite`, `_entity_id_pour` faisant le pont id-registre↔id-GameData en sens INVERSE de `SpinePersonnagesData.par_id`) et instancie le VRAI `CombatCtbUi` dessus — reconstruit ENTIÈREMENT au premier lancement et à chaque changement de CRÉATURE (`←/→`, `_lancer_duel` : ça change les stats réelles, donc le combattant du moteur lui-même). `↑/↓`/`H`/`V` (palier prévisualisé, niveau/costume du héros) ne changent QUE le skin affiché, jamais les stats — HOT-RELOAD via `CombatCtbUi.previsu_rafraichir_visuel(pour_ennemi)` : remplace juste le sprite+l'ombre du combattant concerné (factorisé dans `CombatCtbUi._construire_visuel`, appelé aussi par la construction initiale — zéro logique dupliquée) sans reconstruire l'écran ni toucher au moteur/à l'état du combat en cours (retour Rhend : « t'es obligé de reload le combat ? »). Mode USINE = simple bascule de visibilité (décor de l'Usine seul, `CombatDecorFactory`, masques retirés), le duel n'est PAS détruit. Navigation propre à la vitrine (surcharges dev sur `CombatCtbUi`, ignorées par le jeu réel — voir plus haut) : `←/→` créature (reconstruction), `↑/↓` palier prévisualisé (hot-reload), `H` niveau d'équipement du héros (hot-reload, 1..6), `V` accessoire de visage (hot-reload, jeux `cosmetiques` du registre), `B` éclairage (`CombatCtbUi.previsu_definir_voile`, 4 niveaux Nuit/Studio/Jour/Blanc, défaut Studio), `Tab` bascule USINE, `F1` mémo des commandes ci-dessus (MASQUÉ par défaut). Le reste — Attaquer/Défendre/Compétence/ciblage, file d'initiative, zoom-duel, animations — est le VRAI `CombatCtbUi`, zéro code dupliqué ; son rendu est couvert par `TestCombatUi.gd`, `TestShowRoom.gd` ne teste plus que le câblage propre à la vitrine (surcharges transmises, reconstruction, bascule Usine, voile). Mode LIBRE (toutes les créatures en rangées comparatives) SUPPRIMÉ à la fusion — pas d'équivalent en combat réel. Contenu lu du registre `data/personnages/spine_personnages.tres` (`SpinePersonnagesData`) : **un monstre livré = une entrée, rien d'autre à toucher**. Captures : `SHOT_MODE=showroom` (ScreenshotTool) | `scenes/showroom/ShowRoom.gd` |
| Assets Spine des ENNEMIS (08/2026) : `assets/personnages/ennemis/<id>/` — FlameBot + WorkBot livrés. MÊME contrat d'animation que Relic (`Idle`/`Attack_CaC`/`Hit`/`Death`), et les 5 PALIERS sont des **skins Spine** `<Nom>_Nv1`…`_Nv5` (Nv1 = Commun … Nv5 = Légendaire) commutées par `set_skin_by_name` + `set_slots_to_setup_pose` (sans le 2ᵉ appel la pose garde l'ancienne apparence). `creer()` prend skel/atlas/**apparence**/hauteur, Relic restant le défaut. Inspecter une livraison : `godot --headless --path . res://tools/inspect_spine_ennemis.tscn` (scène, même raison que verif_spine). Les apparences d'une entrée passent par `SpinePersonnagesData.apparences(entree, cosmetique)` : `variantes` (liste nommée) > `niveaux` (slots `_Nv<n>`) > `prefixe_skin` (5 paliers en skins) > apparence unique — un appelant n'a jamais à savoir laquelle ; il passe le Dictionary produit à `SpriteSpinePersonnage.definir_apparence()`. **RELIC — livraison « costumes » du 24/08/2026** : il n'a PAS une skin par apparence. Ses skins se CUMULENT (`skins_base` = corps `Men_Global` + équipement `Men_Level`/`Men_Level_Hit` + vêtement, plus UN jeu de `cosmetiques` — les accessoires de visage « Random », mutuellement exclusifs) en une skin composée (`SpineSprite.new_skin` + `add_skin`), et ses **6 paliers d'équipement sont portés par des SLOTS** suffixés `_Nv1`…`_Nv6` (`R_H_Idle_Tete_Nv3`, et des pièces mutualisées `…_Nv4/5`, `…_Nv1_2_3` — lecture : `SpriteSpinePersonnage.niveaux_du_slot`). Poser un niveau = PURGER la skin composée des slots des autres niveaux (`remove_attachment`) : vider les slots ne suffirait PAS, une animation qui repose un attachement le retrouverait dans la skin et rallumerait la pièce. Les versions masculine/féminine N'ONT PAS été livrées ; le jour où elles arrivent, remplir `variantes` dans le `.tres` suffit. **SENS D'EXPORT** (`regarde_a_droite`, 27/08/2026) : le registre décrit L'ASSET, l'appelant décide de la MISE EN SCÈNE. Christophe dessine CHAQUE CAMP PRÊT À L'EMPLOI — Relic vers la DROITE, FlameBot et WorkBot vers la GAUCHE : posés à leurs ancrages (joueur 0.25, adverse 0.75) ils se font face SANS AUCUN MIROIR. Le code appliquait pourtant un `scale = Vector2(-1, 1)` en dur à TOUT ennemi, ce qui les retournait DOS à Relic — bug signalé par Christophe, corrigé le 27/08/2026. ⚠ **Ne JAMAIS lire le sens sur l'ARME** : le FlameBot porte son canon en travers du corps, pointé vers l'arrière-droite, alors que sa TÊTE (masque en V, optiques) regarde à gauche — c'est cette confusion qui a produit le bug. Le REGARD donne le sens. Désormais `SpinePersonnagesData.echelle_x(entree, doit_regarder_a_droite)` donne le signe (+1 partout aujourd'hui) et `SpriteSpinePersonnage.orienter(signe)` ne touche que le SIGNE de `scale.x` (une taille posée par l'appelant survit). Une livraison qui changerait de sens = une ligne sur SON entrée, zéro code. **Ennemis branchés au combat réel (09/2026)** : `CombatCtbUi._creer_sprite_ennemi` cherche l'id du combattant (`CombattantCtbData.id`, = id GameData, ex. `creature_flamebot`) dans le registre via `SpinePersonnagesData.par_id` (accepte l'id GameData ET l'id court du registre — pas de préfixe `creature_` à dupliquer), pose l'apparence au palier de la **Maîtrise RÉELLE** de la créature (`GameData.get_entity(id).maitrise_actuelle`, mêmes indices que les 5 skins de palier) et appelle `orienter()` comme la ShowRoom. C'est GÉNÉRIQUE à toute future créature/Lieu : `{}` (registre sans entrée pour cet id) → repli automatique sur le placeholder `EnergyBoule`, aucune liste d'ids en dur à tenir à jour. Le héros de combat est monté au **Nv1 (Commun, la dotation de départ)** par `SpriteSpinePersonnage.creer_heros(niveau)` : brancher le niveau RÉEL sur l'équipement du joueur se fait à cet appel, et nulle part ailleurs | `scripts/resources/SpinePersonnagesData.gd` |
...
godot --headless --path . res://tests/TestShowRoom.tscn         # vitrine Spine (= CombatCtbUi fusionné) : registre, aller-retour QG, éclairage, costumes de Relic, échelle, bake des silhouettes, sens d'export (108)
