...
#   godot --headless --path . --script res://tools/verif_spine.gd
extends SceneTree
...
func _init() -> void:
	...
	if sprite == null:
		push_error("creer() a rendu null — runtime ou assets KO")
		ok = false
	else:
		...
		if spine == null:
			...
			for attendu in [SpriteSpinePersonnage.ANIM_IDLE, SpriteSpinePersonnage.ANIM_ATTACK,
					SpriteSpinePersonnage.ANIM_HIT, SpriteSpinePersonnage.ANIM_DEATH]:
